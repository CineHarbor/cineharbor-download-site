...
- 本仓库以 `CineHarbor/cineharbor-download-site` 作为 Project 身份，identity `urn:cineharbor:project:cineharbor-download-site`。
...
- 发行数据由 `CineHarbor/cineharbor-desktop` GitHub Releases 聚合注入。
...
- 兼容线声明改为 Core `1.0` / `repository-filesystem/1.0`；新增 `continuity.lineage: "urn:cineharbor:lineage:cineharbor-download-site"`（0.1 唯一隐式连续线的显式初始 lineage）。
